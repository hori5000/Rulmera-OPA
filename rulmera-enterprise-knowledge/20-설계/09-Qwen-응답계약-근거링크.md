...
따라서 Qwen의 출력은 **Rulmera Answer Contract**로 제한한다.
...
음성은 Input Adapter일 뿐 답변 경로는 같다.

```mermaid
flowchart LR
  MIC[Microphone] --> STT[STT]
  STT --> TXT[Canonical Text Question]
  TXT --> OPA[OPA]
  OPA --> RAG[RAG]
  RAG --> QW[Qwen]
  QW --> JSON[Answer Contract]
  JSON --> UI[Rulmera Template]
```
