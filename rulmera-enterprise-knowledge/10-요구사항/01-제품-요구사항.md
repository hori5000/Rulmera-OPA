...
> **지식의 수집 → 자동분류 → 검토/승인 → 정본화 → 권한판정 → 검색 → Qwen 답변 → 근거/원문 확인 → 감사 → 지속 갱신**과 **Qwen 계획 → Action OPA → Worker 실행 → 검증 → 결과회수**를 하나의 제품 안에서 제공하는 Enterprise Knowledge OS.
...
6. **Qwen이 직접 실행권한을 가지지 않는다.**  
   Action OPA와 Worker Controller를 거쳐 승인된 Job만 Execution Worker가 수행한다.
...
| **Jobs / Workers** | AI가 생성한 작업, 실행상태, 변경파일, 검증결과, 승인대기 확인 |
...
- Worker Registry / Tool Allowlist 관리
- Job Queue / Retry / Dead Letter 관리
- Action Approval Gate / Result Audit 관리
...
## 실행형 Worker 요구사항
상세 기준은 [[06-Agentic-Worker-요구사항]]을 따른다.
